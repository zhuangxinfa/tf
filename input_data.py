...
import os
train_dir = 'C:/Users/zhuan/Desktop/tf/pic/test_data'  
...
#        mao.append(file_dir +'/mao'+'/'+ file)   
#        label_mao.append(0)  
#    for file in os.listdir(file_dir+'/gou'):  
#        gou.append(file_dir +'/gou'+'/'+file)  
#        label_gou.append(1)  
    for file in os.listdir(file_dir+'/mao'):  
        mao.append(file_dir +'/mao'+'/'+ file)    
        label_mao.append(0)  
...
#step2：对生成的图片路径和标签List做打乱处理把cat和dog合起来组成一个list（img和lab） 
    image_list = np.hstack((mao, gou))      #它其实就是水平(按列顺序)把数组给堆叠起来，vstack()函数正好和它相反。
#    print(image_list)
    #image_list = np.hstack((husky, jiwawa, poodle, qiutian))  
    label_list = np.hstack((label_mao, label_gou))  #它其实就是水平(按列顺序)把数组给堆叠起来，vstack()函数正好和它相反。
#    print (label_list)
    #利用shuffle打乱顺序  
